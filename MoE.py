...
class MixtureOfExperts:
    """
    Gating network that learns per-sample weights for each expert model.
    Features used: recent volatility, RSI, MACD signal, short/long SMA spread.
    """
    ...
    def _extract_gating_features(self, data: pd.DataFrame) -> np.ndarray:
        """Build a compact feature vector the gating net uses to weight experts."""
        feats = pd.DataFrame()
        feats['volatility'] = data['Close'].pct_change().rolling(5).std()
        feats['rsi']        = data['RSI_14'] / 100.0
        feats['macd_signal']= data['MACDs_12_26_9']
        feats['sma_spread'] = (data['SMA10'] - data['SMA20']) / data['Close']
        feats['momentum']   = data['Close'].pct_change(5)
        feats.dropna(inplace=True)
        return feats.values

    def train_gating_network(self, data: pd.DataFrame,
                              model_predictions: dict,
                              actual_prices: np.ndarray,
                              training_data_len: int):
        """
        Fit a ridge-regression gating net whose outputs (after softmax)
        become the mixing weights.
        """
        from sklearn.linear_model import Ridge
        from scipy.special import softmax

        gating_feats = self._extract_gating_features(data)
        # Align gating features with the test window
        gating_test = gating_feats[training_data_len - len(gating_feats):]
        min_len = min(len(gating_test), len(actual_prices))
        gating_test   = gating_test[:min_len]
        actual_trimmed = actual_prices[:min_len]

        # Build target matrix: how much better is each expert vs the mean?
        pred_matrix = np.column_stack([
            v[:min_len] for v in model_predictions.values()
        ])
        errors = np.abs(pred_matrix - actual_trimmed[:, None])
        # Lower error → higher target weight (inverted + softmax)
        inv_errors = 1.0 / (errors + 1e-8)
        target_weights = inv_errors / inv_errors.sum(axis=1, keepdims=True)

        # One Ridge regressor per expert
        self.gating_models = []
        for i in range(self.n_experts):
            clf = Ridge(alpha=1.0)
            clf.fit(self.feature_scaler.fit_transform(gating_test),
                    target_weights[:, i])
            self.gating_models.append(clf)

        self._gating_feats_cache = gating_test
        self._min_len = min_len

    def predict(self, data: pd.DataFrame,
                model_predictions: dict,
                training_data_len: int) -> tuple[np.ndarray, np.ndarray]:
        """
        Returns (moe_prediction, weight_matrix).
        weight_matrix shape: (n_samples, n_experts).
        """
        from scipy.special import softmax
        ...
        scaled = self.feature_scaler.transform(gating_test)
        raw_weights = np.column_stack([
            m.predict(scaled) for m in self.gating_models
        ])
        # Softmax so weights sum to 1 per sample
        weights = softmax(raw_weights, axis=1)

        pred_matrix = np.column_stack([
            v[:min_len] for v in model_predictions.values()
        ])
        moe_pred = (pred_matrix * weights).sum(axis=1)
        return moe_pred, weights
...
if st.sidebar.button("Run Prediction", type="primary"):
    ...
    if data is not None:
        ...
        if len(results) >= 2:
            status_text.text("Training Mixture of Experts gating network...")
            try:
                moe = MixtureOfExperts(n_experts=len(results))
                moe.train_gating_network(data, results, actual_prices, training_data_len)
                moe_predictions, moe_weights = moe.predict(data, results, training_data_len)
                results["MoE"] = moe_predictions

                # Show per-expert weights as a small table
                weight_df = pd.DataFrame(
                    moe_weights,
                    columns=list(results.keys())[:-1]   # exclude MoE itself
                )
                with st.expander("MoE gating weights per prediction day"):
                    st.dataframe(
                        weight_df.style.format("{:.3f}").background_gradient(
                            cmap="YlOrRd", axis=1
                        ),
                        use_container_width=True
                    )
            except Exception as e:
                st.error(f"MoE ensemble error: {str(e)}")
